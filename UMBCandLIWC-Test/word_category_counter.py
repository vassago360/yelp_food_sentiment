...
def default_dictionary_filename():
    return os.path.abspath(os.path.join(os.path.dirname(__file__), 'LIWC2007.dic'))
...
if __name__ == '__main__':
    ...
    if not 2 <= len(sys.argv) <= 3:
        ...
        print('Sample:')
        txt = """It's 3 o'clock. I like cake."""
        print(txt)
        print(score_text(txt))
        print()
        sys.stderr.write("Usage: word_category_counter.py filename_to_process [dictionary_filename.dic]\n")
    else:
        filename = sys.argv[1]
        if len(sys.argv) > 2:
