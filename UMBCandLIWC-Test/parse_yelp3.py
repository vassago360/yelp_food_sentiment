...
def LRTest(NNs, clf, bootstrapIteration, maxPos=60000, maxNeg=60000): #restID foodItem FoodItemVec Sent SentVecs
...
if __name__ == "__main__":
    ...
    posSeeds = makeDictAFiveLists(dictPosRestIDToFoodItemFoodItemVecSentSentVec)
    negSeeds = makeDictAFiveLists(dictNegRestIDToFoodItemFoodItemVecSentSentVec)
    print "  initial Pos Seeds added:", len(posSeeds)
    print "  initial Neg Seeds added:", len(negSeeds)
    ...
    while(i < 20):
        ...
        posSeeds = makeDictAFiveLists(dictPosRestIDToFoodItemFoodItemVecSentSentVec)
        negSeeds = makeDictAFiveLists(dictNegRestIDToFoodItemFoodItemVecSentSentVec)
        map(itemgetter(4), posSeeds) + map(itemgetter(4), posSeeds)
        [1]*len(posSeeds)+[0]*len(negSeeds)
        clf = logistic.fit( map(itemgetter(4), posSeeds) + map(itemgetter(4), posSeeds), [1]*len(posSeeds)+[0]*len(negSeeds)) #could be faster not training on so many
        print "iteration", i 
        #get NNs of new pos/neg seeds --kind of like knn compare get-all-sentVec with each-new-seed-sentVec )
        #w2vNNs = []
        w2vNNs = getReviewSentencesNNs(dictRestIDToFoodItemFoodItemVecSentSentVec, posSeeds+negSeeds)
        ...
        #sameFINNs = []
        sameFINNs = addSentsThatTalkAboutThatFoodItemInRestAutoIntoSeeds(dictRestIDToFoodItemFoodItemVecSentSentVec, posSeeds, negSeeds)
